...
app.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const activeClients = Object.values(status.clients || {}).filter((client: any) => client.status === 'connected').length;
    const outages = status.events?.filter((event: any) => event.type === 'reboot').length || 0;
    ...
    res.type('html').send(`
      ...
      <body>
        ...
        <div class="clients-grid">
          ${Object.entries(status.clients || {}).map(([clientId, client]: [string, any]) => `
        ...
        <div class="events-list">
          ...
          ${(status.events || []).slice(-10).reverse().map((event: any) => `
